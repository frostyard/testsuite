...
2. Add or update existing `.feature` files under `tests/smoke/features/`. The
   lab also reserves `system` and `sysext` for their documented contracts; follow
   the copyable [scenario-authoring example](docs/authoring-scenarios.md) when
   creating either suite's first feature or adding step vocabulary.
   `tests/e2e/` is an ACMM discovery marker, not a Behave suite. Keep step
   vocabulary in `tests/shared/steps.py` and probe helpers in
   `tests/shared/host.py` where possible, rather than duplicating logic per
   suite.
